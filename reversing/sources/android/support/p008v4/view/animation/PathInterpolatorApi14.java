...
class PathInterpolatorApi14 implements Interpolator {
    ...
    /* renamed from: mX */
    private final float[] f104mX;

    /* renamed from: mY */
    private final float[] f105mY;
    ...
    public PathInterpolatorApi14(Path path) {
        ...
        this.f104mX = new float[i];
        this.f105mY = new float[i];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < i; i2++) {
            pathMeasure.getPosTan((i2 * length) / (i - 1), fArr, null);
            this.f104mX[i2] = fArr[0];
            this.f105mY[i2] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f104mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f104mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f104mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f105mY[i];
        }
        float[] fArr2 = this.f105mY;
        float f3 = fArr2[i];
